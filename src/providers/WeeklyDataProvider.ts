...
export class WeeklyDataProvider implements IDataProvider {
  ...
  private static readonly CATEGORY_TO_MODULE: Record<
    string,
    EnhancedModuleName
  > = {
    // weeklyUpdates
  ...
  private async collectClassicData(
    ...
    await this.syncRecommendationFlagsFromHistory(
      weekStart,
      articleConfig,
      toolConfig,
      {
    ...
    const toolCandidates = await this.toolFilter.filter({
      perCategory: Math.max(
        toolPerCategory,
        toolPerCategory * toolPoolMultiplier,
      ),
    ...
    metadata.showReferenceLinks = this.resolveReferenceLinksSetting(
      this.config,
    );
  ...
  private async collectEnhancedData(
    ...
    this.applyImageBudget(
      modules,
      (enhancedConfig.content as Record<string, any> | undefined)?.images,
    );
    ...
    metadata.showReferenceLinks =
      this.resolveReferenceLinksSetting(enhancedConfig);
  ...
  private routeArticleToModule(
    ...
    const moduleName = this.resolveModuleByCategory(
      article.category,
      "reading",
    );
  ...
  private applyModuleSettings(
    ...
    const applyFilter = <T extends Record<string, any>>(
      ...
      const mapped: Array<ContentItem & { __index: number }> = items.map(
        (item, index) => ({
      ...
      const keep = new Set(
        filtered.map((item) => (item as any).__index as number),
      );
    ...
    for (const moduleName of moduleNames) {
      ...
      if (!criteria) {
        if (moduleConfig[moduleName]?.showImages === false) {
          this.stripModuleImages(
            modules[moduleName] as Array<Record<string, any>>,
          );
        }
        continue;
      }
      (modules[moduleName] as any[]) = applyFilter(
        modules[moduleName] as any[],
        criteria,
      );
      if (moduleConfig[moduleName]?.showImages === false) {
  ...
  private async scanDataSourceFiles(
  ...
  private toExerciseRecordFromArticle(
    article: EnhancedArticle,
  ): ExerciseRecord {
    return {
      type: article.title,
  ...
  private async applyAISummaries(
    modules: EnhancedTemplateData["content"],
  ): Promise<void> {
    ...
    const configuredMax = Number(
      aiConfig?.summaries?.maxLength || aiConfig?.maxLength || 180,
    );
      ...
      if (!forceAI && article.aiSummary && article.aiSummary.trim()) {
        ...
        try {
          ...
          if (sourceContent && aiProvider) {
            try {
              const aiPrompt = this.buildAISummaryPrompt(
                article,
                sourceContent,
                minLength,
                maxLength,
          ...
          article.aiSummary = this.buildPersonalReflectionFallback(
            article,
            minLength,
            maxLength,
          );
  ...
  private createSummaryFallback(
    ...
    if (!compact) {
      ...
      return this.normalizeSummaryLength(
        fallback,
        minLength,
        maxLength,
        seedText,
    ...
    if (this.isLikelyEnglish(normalized) && article) {
      return this.buildPersonalReflectionFallback(
        article,
        minLength,
        maxLength,
        compact,
  ...
  private normalizeSummaryLength(
    ...
    compact = compact
      .replace(/^(摘要|总结|Summary|TL;DR)[:：\-\s]+/i, "")
      .trim();
    if (!compact) {
      compact = "暂无可提取摘要。";
    }
  ...
  private composeDeepSummary(content: string): string {
    ...
    if (
      midCandidate &&
      !selected.some((item) => item.index === midCandidate.index)
    ) {
    ...
    if (
      tailCandidate &&
      !selected.some((item) => item.index === tailCandidate.index)
    ) {
  ...
  private async generateAIReflectionSummary(
    ...
    try {
      const basePrompt = this.buildAISummaryPrompt(
        article,
        sourceContent,
        minLength,
        maxLength,
  ...
  private buildPersonalReflectionFallback(
    ...
    const compact = sourceContent
      ? this.stripMarkdownForSummary(sourceContent)
      : "";
    ...
    const insightBody =
      keyPoints.length > 0
        ? keyPoints.join("，")
        : genericInsights[seed % genericInsights.length];
  ...
  private decorateReadingModules(
    modules: EnhancedTemplateData["content"],
  ): void {
    const reading = (modules.reading || []) as EnhancedArticle[];
    const readingBooks = reading.filter((item) =>
      this.isBookLikeReadingItem(item),
    );
    const readingArticles = reading.filter(
      (item) => !this.isBookLikeReadingItem(item),
    );
  ...
  private buildDefaultUnsplashQuery(
    ...
    const selected =
      weighted.length > 0 ? weighted.slice(0, 3) : ["productivity", "desk"];
    const topic =
      kind === "cover"
        ? selected
        : ["background", "light", ...selected.slice(0, 1)];
  ...
  private buildUnsplashUrl(
    ...
    const fallbackKeywords =
      keywords.length > 0 ? keywords : ["minimal", "creative"];
    const selectedAsset = this.selectUnsplashAsset(
      kind,
      fallbackKeywords,
      weekStart,
    );
  ...
  private selectUnsplashAsset(
    ...
    const candidates = WeeklyDataProvider.UNSPLASH_ASSETS.filter((asset) =>
      asset.kinds.includes(kind),
    ).map((asset) => ({
      asset,
    ...
    const pool = (matched.length > 0 ? matched : candidates).map(
  ...
  private normalizeModuleImages(
    ...
    for (const moduleName of moduleNames) {
      const items = modules[moduleName] as
      ...
      for (const item of items) {
        ...
        const fallbackDomainIcon = allowFallbackIcon
          ? this.buildDomainIconUrl(itemUrl)
          : undefined;
        item.image =
          normalizedImage ||
          normalizedCover ||
  ...
  private applyImageBudget(
    ...
    const priorityModules = this.resolveImagePriority(
      imageConfig?.keepPriority,
    );
    ...
    const selectFromModule = (moduleName: EnhancedModuleName): void => {
      ...
      for (let index = 0; index < items.length; index += 1) {
        ...
        selectedPerModule.set(
          moduleName,
          (selectedPerModule.get(moduleName) || 0) + 1,
        );
        selectedCount += 1;
      }
    };
  ...
  private pruneWeeklyUpdates(modules: EnhancedTemplateData["content"]): void {
    ...
    modules.weeklyUpdates = updates.filter(
      (item) => !this.isPublishingRecord(item as Record<string, any>),
    );
  }
  ...
  private normalizeImageUrl(
    ...
    if (parsed.hostname.toLowerCase() === "logo.clearbit.com") {
      ...
      const domainFromPath =
        pathHost && this.looksLikeDomain(pathHost) ? pathHost : undefined;
    ...
    if (parsed.hostname.toLowerCase() === "source.unsplash.com") {
      return this.buildUnsplashUrl(
        "cover",
        "minimal,creative,workspace",
        new Date(),
      );
    }
  ...
  private createAIProvider(
    ...
    const aiEnabled = Boolean(
      aiConfig?.enabled ??
      aiConfig?.summaries?.enabled ??
      aiConfig?.goldenQuote?.enabled,
    );
    const quoteEnabled = Boolean(aiConfig?.goldenQuote?.enabled);
    const shouldEnableForPurpose =
      purpose === "summaries" ? aiEnabled : aiEnabled || quoteEnabled;
    if (!shouldEnableForPurpose || !aiConfig?.provider) {
      if (purpose === "summaries" && aiConfig?.summaries?.enabled) {
        console.warn(
          "[weekly] summaries 已启用，但未开启 ai.enabled 或 provider，已跳过 AI 摘要生成。",
        );
      }
      return null;
    }
    ...
    if (!supported.includes(provider)) {
      console.warn(
        `[weekly] 当前不支持 provider=${providerRaw}，已忽略。支持: ${supported.join(", ")}`,
      );
      return null;
    }
    ...
    const providerConfig: AIProviderConfig = {
      ...
      baseUrl:
        typeof aiConfig.baseUrl === "string"
          ? aiConfig.baseUrl
          : process.env.AI_BASE_URL,
      apiKey,
      timeout:
        typeof aiConfig.timeout === "number" ? aiConfig.timeout : undefined,
      maxRetries:
        typeof aiConfig.maxRetries === "number"
          ? aiConfig.maxRetries
          : undefined,
    };
  ...
  private buildGoldenQuotePromptSource(
    ...
    const music = (modules.music || [])
      ...
      .map(
        (item) =>
          `${item.title} - ${item.artist}${item.feeling ? `：${item.feeling}` : ""}`,
      );
  ...
  private async syncRecommendationFlagsFromHistory(
    ...
    const capturesConfig = (contentConfig.captures || {}) as Record<
      string,
      any
    >;
    const foodConfig = (contentConfig.food || {}) as Record<string, any>;
    const exerciseConfig = (contentConfig.exercise || {}) as Record<
      string,
      any
    >;
    ...
    const syncLife =
      isEnhanced && Boolean(lifeConfig.syncRecommendedFromHistory);
    const syncCaptures =
      isEnhanced && Boolean(capturesConfig.syncRecommendedFromHistory);
    const syncFood =
      isEnhanced && Boolean(foodConfig.syncRecommendedFromHistory);
    const syncExercise =
      isEnhanced && Boolean(exerciseConfig.syncRecommendedFromHistory);
    const syncMusic =
      isEnhanced && Boolean(musicConfig.syncRecommendedFromHistory);
    const syncMovies =
      isEnhanced && Boolean(moviesConfig.syncRecommendedFromHistory);
    ...
    const historyUrls = await this.collectWeeklyHistoryUrls(
      weekStart,
      historyDays,
    );
    if (historyUrls.size === 0) {
      console.log(
        "[weekly] 未检测到上一周推荐记录，跳过标记同步，直接生成本周 weekly 内容...",
      );
      return;
    }
    ...
    if (
      ...
      if (syncLife) {
        parts.push(
          `life=${lifeStats.updatedEntries} (files=${lifeStats.updatedFiles})`,
        );
      }
      if (syncCaptures) {
        parts.push(
          `captures=${capturesStats.updatedEntries} (files=${capturesStats.updatedFiles})`,
        );
      }
      if (syncFood) {
        parts.push(
          `food=${foodStats.updatedEntries} (files=${foodStats.updatedFiles})`,
        );
      }
      if (syncExercise) {
        parts.push(
      ...
      if (syncMusic) {
        parts.push(
          `music=${musicStats.updatedEntries} (files=${musicStats.updatedFiles})`,
        );
      }
      if (syncMovies) {
        parts.push(
          `movies=${moviesStats.updatedEntries} (files=${moviesStats.updatedFiles})`,
        );
      }
      if (syncTV) {
        parts.push(
          `tv=${tvStats.updatedEntries} (files=${tvStats.updatedFiles})`,
        );
      }
  ...
  private async syncArticleRecommendedFlags(
    ...
    const articleSources =
      this.config.sources.articles || this.config.sources.clippings;
  ...
  private async syncToolRecommendedFlags(
    ...
    for (const filePath of files) {
      try {
        ...
        const parsed = matter(raw);
        const data = parsed.data as Record<string, any>;
        // 支持 tools / sources / items 三种 key
        const listKey = Array.isArray(data.tools)
          ? "tools"
        ...
        let changed = false;
        data[listKey] = (data[listKey] as unknown[]).map((entry: unknown) => {
          ...
          const url =
            typeof toolData.url === "string" ? toolData.url : undefined;
          const nextRecommended = this.hasBeenRecommendedByUrl(
            url,
            historyUrls,
          );
  ...
  private async collectFilesFromSources(
    sourceInput: DataSourceInput | undefined,
  ): Promise<string[]> {
    if (!sourceInput) {
      return [];
  ...
  private async filterArticlesByWeeklyHistory(
    ...
    const seenUrls = await this.collectWeeklyHistoryUrls(
      weekStart,
      historyDays,
    );
    if (seenUrls.size === 0) {
